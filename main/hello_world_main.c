...
static spi_bus_config_t lcd_st7789_buscfg = {
    ...
    .max_transfer_sz =
        LCD_H_RES * 80 *
        sizeof(uint16_t), // transfer 80 lines of pixels (assume pixel is RGB565) at most in one SPI transaction
};
...
static void app_main_UI_starter(void* params)
{
    ...
    lv_indev_set_display(touch_screen_device, display);
    // lv_indev_add_event_cb(touch_screen_device, screen_touch_event_handler, LV_EVENT_PRESSED, NULL);
    lv_indev_set_user_data(touch_screen_device, tp);
